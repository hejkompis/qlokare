...
echo '<h2>Reported messages</h2>';
echo '<p>';
echo $wpc->get_counts('reported') > 0 ? '' : 'You don\'t have any reported messages for the now.';
echo '</p>';
foreach( $query as $q ) { // delete the report (saved as user meta) if the message no longer exists
...
if( $wpc->get_counts('reported') > 0 ) {
	$umetas = '';
	$msgs 	= '';
	echo '<p>From this area you will be able to access and moderate user-reported messages. Actions:';
	echo '<li><code>delete message</code>: Deletes message forever for both sender and recipient.</li>';
	echo '<li><code>delete report</code>: This option will only delete the report, and won\'t take any other action.</li>';
	//echo '<li><code>contact {user} (sender)</code>: If the moderator wants to send a message to the user who has sent the reported message.</li>';
	//echo '<li><code>contact {user} (reporter)</code>: If the moderator wants to send a message to the user who has reported the message, to say thanks, maybe :)</li>';
	echo $wpchats->get_settings('mod_can_view_chats') ? '<li><code>view conversation</code>: If you need to investigate the reported message, then, you can view the conversation where it was reported from.</li>' : '';
	if($wpchats->get_settings('mod_can_ban') || current_user_can('manage_options')):;
	echo '<li><code>ban {user} from chat</code>: Attention - this option will block the user from using chat forever. Don\'t worry, you can unban any user ';
	echo $exit ? 'below.' : 'from <a href="admin.php?page=wpchats&section=banned">this section.</a>';
	echo '</li>';
	endif;
	echo '</p>';
	...
	echo '<tr><th>Sender</th> <th>Recipient</th> <th>Message body</th> <th>Time</th> <th>Deleted by</th> <th>Reported by</th> <th>Reason</th> <th>Action</th> </tr>';
	foreach ( $query as $q ) {
		...
		if( $wpchats->get_message_parts($id, 'chat_id') && ! $noReturn ) {
			...
			$msgs 	.= $id. ',';
			echo '<tr>';
			echo '<td>'.$wpc->get_user_icon($wpchats->get_message_parts($id, 'from'), 30, $href).'</td>';
			echo '<td>'.$wpc->get_user_icon($wpchats->get_message_parts($id, 'to'), 30, $href).'</td>';
			echo '<td>'.$wpchats->get_message_parts($id, 'body').'</td>';
			echo '<td>'.$wpchats->time_difference( date("Y-m-d H:i:s", $wpchats->get_message_parts($id, 'time')), date("Y-m-d H:i:s", time()), ' ago' ).'</td>';
			echo '<td>';
			...
			echo '</td>';
			echo '<td>'.$wpc->get_user_icon($q->user_id, 30, $href).'</td>';
			echo '<td>'.$q->meta_value.'</td>';
			echo '<td>';
