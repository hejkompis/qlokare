...
function wpc_create_menu() {
	$wpc 		= new wpc;
	$span_1 	= !isset($_GET["section"]) && isset($_GET["page"]) && $_GET["page"] == 'wpchats' ? '<span class="active">WpChats</span>' : 'WpChats';
	$span_2 	= isset($_GET["section"]) && $_GET["section"] == 'settings' ? '<span class="active">Settings</span>' : 'Settings';
	$span_3 	= isset($_GET["section"]) && $_GET["section"] == 'moderate' ? '<span class="active">Moderation</span>' : 'Moderation';
	$span_3		.= $wpc->get_counts('reported') > 0 ? ' <span class="menu_count">'.$wpc->get_counts('reported').'</span>' : '';
	$span_4 	= isset($_GET["section"]) && $_GET["section"] == 'emoji' ? '<span class="active">Emoji</span>' : 'Emoji';
	$span_5 	= isset($_GET["section"]) && $_GET["section"] == 'banned' ? '<span class="active">Banned users</span>' : 'Banned users';
	$span_6 	= isset($_GET["section"]) && $_GET["section"] == 'deleted' ? '<span class="active">Deleted messages</span>' : 'Deleted messages';
	$span_7 	= isset($_GET["section"]) && $_GET["section"] == 'mailing' ? '<span class="active">Mailing</span>' : 'Mailing';
	$span_8 	= isset($_GET["section"]) && $_GET["section"] == 'translate' ? '<span class="active">Translate</span>' : 'Translate';
	add_menu_page( 'WpChats', $span_1, 'manage_options', 'wpchats', 'wpc_admin_page' );
	add_submenu_page( 'wpchats', 'Moderation', $span_3, 'manage_options', 'admin.php?page=wpchats&section=moderate');
	add_submenu_page( 'wpchats', 'Emoji', $span_4, 'manage_options', 'admin.php?page=wpchats&section=emoji');
	add_submenu_page( 'wpchats', 'Settings', $span_2, 'manage_options', 'admin.php?page=wpchats&section=settings');
	add_submenu_page( 'wpchats', 'Deleted messages', $span_6, 'manage_options', 'admin.php?page=wpchats&section=deleted');
	add_submenu_page( 'wpchats', 'Banned users', $span_5, 'manage_options', 'admin.php?page=wpchats&section=banned');
	add_submenu_page( 'wpchats', 'Mailing', $span_7, 'manage_options', 'admin.php?page=wpchats&section=mailing');
	add_submenu_page( 'wpchats', 'Translate', $span_8, 'manage_options', 'admin.php?page=wpchats&section=translate');
...
add_action('admin_enqueue_scripts', function() {
		wp_enqueue_script('wpc-admin-jquery', WPC_PLUGIN_DIR . 'admin/inc/js/global.js', false, null );
		wp_enqueue_script('wpc-admin-js', WPC_PLUGIN_DIR . 'admin/inc/js/functions.js' );
		wp_enqueue_style('wpc-admin', WPC_PLUGIN_DIR . 'admin/inc/css/style.css' );
