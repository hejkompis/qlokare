    ...
    if (is_dir($dirname)) {
        ...
        if ($subdir)
        {
                ...
                foreach($ptmp as $item)
                {
                        ...
                        {
                                $sp[] = $item;
                                $subdir_path.= '/<a href="' . esc_url( add_query_arg( array( 'd' => base64_encode('/' . implode('/', $sp)) ) ) ) . '">' . m1dll_utf8_encode( $item ) . '</a>';
                        }
                }
        }
        ...
        if ($dh = opendir($dirname)) 
        {
                ...
                while (($file = readdir($dh)) !== false) 
                {
                        if ($file != "." && $file != "..")
                        {
                                ...
                                {
                                        //folder
                                        $href = add_query_arg( array( 'd' =>  rawurlencode( base64_encode($subdir.'/'.$file) ) ) );
