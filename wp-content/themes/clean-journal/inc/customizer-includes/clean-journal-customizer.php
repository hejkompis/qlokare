...
function clean_journal_customize_register( $wp_customize ) {
	...
	//Custom Controls
	require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-custom-controls.php';
	...
	$wp_customize->add_control( 'clean_journal_theme_options[logo_disable]', array(
		'label'    => __( 'Check to disable logo', 'clean-journal' ),
		'priority' => 101,
		'section'  => 'title_tagline',
	...
	$wp_customize->add_control( 'clean_journal_logo_alt_text', array(
		'label'    	=> __( 'Logo Alt Text', 'clean-journal' ),
		'priority'	=> 102,
		'section' 	=> 'title_tagline',
	...
	$wp_customize->add_control( 'clean_journal_theme_options[move_title_tagline]', array(
		'label'    => __( 'Check to move Site Title and Tagline before logo', 'clean-journal' ),
		'priority' => 103,
		'section'  => 'title_tagline',
	...
	$schemes = clean_journal_color_schemes();
	...
	$wp_customize->add_control( 'clean_journal_theme_options[color_scheme]', array(
		'choices'  => $choices,
		'label'    => __( 'Color Scheme', 'clean-journal' ),
		'priority' => 5,
		'section'  => 'colors',
	...
	require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-header-options.php';

	//Theme Options
	require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-theme-options.php';
	
	//Featured Content Setting
	require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-featured-content-setting.php';
   	
	//Featured Slider
	require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-featured-slider.php';

	//Social Links
	require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-social-icons.php';
	
	// Reset all settings to default
	$wp_customize->add_section( 'clean_journal_reset_all_settings', array(
		'description'	=> __( 'Caution: Reset all settings to default. Refresh the page after save to view full effects.', 'clean-journal' ),
		'priority' 		=> 700,
		'title'    		=> __( 'Reset all settings', 'clean-journal' ),
	) );

	$wp_customize->add_setting( 'clean_journal_theme_options[reset_all_settings]', array(
		'capability'		=> 'edit_theme_options',
		'default'			=> $defaults['reset_all_settings'],
		'sanitize_callback' => 'clean_journal_reset_all_settings',
		'transport'			=> 'postMessage',
	) );

	$wp_customize->add_control( 'clean_journal_theme_options[reset_all_settings]', array(
		'label'    => __( 'Check to reset all settings to default', 'clean-journal' ),
	...
	$wp_customize->add_section( 'important_links', array(
		'priority' 		=> 999,
		'title'   	 	=> __( 'Important Links', 'clean-journal' ),
	) );
	...
	$wp_customize->add_control( new Clean_Journal_Important_Links( $wp_customize, 'important_links', array(
        'label'   	=> __( 'Important Links', 'clean-journal' ),
...
function clean_journal_customize_preview() {
	wp_enqueue_script( 'clean_journal_customizer', get_template_directory_uri() . '/js/clean-journal-customizer.min.js', array( 'customize-preview' ), '20120827', true );
...
function clean_journal_customize_scripts() {
	wp_enqueue_script( 'clean_journal_customizer_custom', get_template_directory_uri() . '/js/clean-journal-customizer-custom-scripts.min.js', array( 'customize-controls', 'iris', 'underscore', 'wp-util' ), '20150630', true );

	$clean_journal_misc_links = array(
							'upgrade_link' 				=> esc_url( 'http://catchthemes.com/themes/clean-journal-pro/' ),
							'upgrade_text'	 			=> __( 'Upgrade To Pro &raquo;', 'clean-journal' ),
							'WP_version'				=> get_bloginfo( 'version' ),
							'old_version_message'		=> __( 'Some settings might be missing or disorganized in this version of WordPress. So we suggest you to upgrade to version 4.0 or better.', 'clean-journal' )
		);
	
	$clean_journal_misc_links['color_list'] = clean_journal_color_list();
	
	//Add Upgrade Button and old WordPress message via localized script
	wp_localize_script( 'clean_journal_customizer_custom', 'clean_journal_misc_links', $clean_journal_misc_links );

	wp_enqueue_script( 'clean_journal_customizer_custom' );

	wp_enqueue_style( 'clean_journal_customizer_custom', get_template_directory_uri() . '/css/clean-journal-customizer.css');
}
...
function clean_journal_color_list() {
	...
	$clean_journal_color_list['background_color']['light']	= $default['background_color'];
	$clean_journal_color_list['background_color']['dark']	= $default_dark['background_color'];
	
	$clean_journal_color_list['header_textcolor']['light']	= $default['header_textcolor'];
	$clean_journal_color_list['header_textcolor']['dark']	= $default_dark['header_textcolor'];

	return $clean_journal_color_list;
}


//Active callbacks for customizer
require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-active-callbacks.php';

//Sanitize functions for customizer
require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer-sanitize-functions.php';