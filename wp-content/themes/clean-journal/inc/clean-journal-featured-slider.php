				...
				<section id="feature-slider">
					<div class="wrapper">
						<div class="cycle-slideshow" 
							...
							// Select Slider
							if ( $sliderselect == 'demo-featured-slider' && function_exists( 'clean_journal_demo_slider' ) ) {
								$clean_journal_featured_slider .=  clean_journal_demo_slider( $options );
							}
							elseif ( $sliderselect == 'featured-page-slider' && function_exists( 'clean_journal_page_slider' ) ) {
								...
								<article class="post demo-image-1 hentry slides displayblock">
									<figure class="slider-image">
										<a title="Slider Image 1" href="'. esc_url( home_url( '/' ) ) .'">
											<img src="'.get_template_directory_uri().'/images/gallery/slider1-1920x800.jpg" class="wp-post-image" alt="Slider Image 1" title="Slider Image 1">
										</a>
									</figure>
								...
								<article class="post demo-image-2 hentry slides displaynone">
									<figure class="Slider Image 2">
										<a title="Slider Image 2" href="'. esc_url( home_url( '/' ) ) .'">
											<img src="'. get_template_directory_uri() . '/images/gallery/slider2-1920x800.jpg" class="wp-post-image" alt="Slider Image 2" title="Slider Image 2">
										</a>
									</figure>
									<div class="entry-container">
										<header class="entry-header">
		...
		while ( $get_featured_posts->have_posts()) : $get_featured_posts->the_post(); $i++;
			...
			<article class="'.$classes.'">
				...
				if ( has_post_thumbnail() ) {
					$clean_journal_page_slider .= '<a title="Permalink to '.the_title('','',false).'" href="' . get_permalink() . '">
						'. get_the_post_thumbnail( $post->ID, 'clean-journal-slider', array( 'title' => esc_attr( $title_attribute ), 'alt' => esc_attr( $title_attribute ), 'class'	=> 'attached-page-image' ) ).'
					</a>';
				}
				else {
					//Default value if there is no first image
					$clean_journal_image = '<img class="pngfix wp-post-image" src="'.get_template_directory_uri().'/images/gallery/no-featured-image-1200x514.jpg" >';
					
					//Get the first image in page, returns false if there is no image
					$clean_journal_first_image = clean_journal_get_first_image( $post->ID, 'clean-journal-slider', array( 'title' => esc_attr( $title_attribute ), 'alt' => esc_attr( $title_attribute ), 'class' => 'attached-page-image' ) );
