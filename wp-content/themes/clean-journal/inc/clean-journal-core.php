...
if ( ! function_exists( 'clean_journal_setup' ) ) :
	...
	function clean_journal_setup() {
		...
		register_nav_menus( array(
			'primary' 		=> __( 'Primary Menu', 'clean-journal' ),
			'secondary' 	=> __( 'Secondary Menu', 'clean-journal' ),
			'header-top'	=> __( 'Header Top Menu', 'clean-journal' ),
		) );
...
function clean_journal_fonts_url() {
	 ...
	 */
	$open_sans = _x( 'on', 'Open Sans font: on or off', 'clean-journal' );

	/* Translators: If there are characters in your language that are not
	 * supported by Droid Sans, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$droid_sans = _x( 'on', 'Droid Sans font: on or off', 'clean-journal' );
...
function clean_journal_scripts() {
	...
	wp_enqueue_style( 'clean-journal-style', get_stylesheet_uri() );

	wp_enqueue_script( 'clean-journal-navigation', get_template_directory_uri() . '/js/navigation.min.js', array(), '20120206', true );

	wp_enqueue_script( 'clean-journal-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.min.js', array(), '20130115', true );
	...
	if ( $options['color_scheme'] != 'light' )
		wp_enqueue_style( 'clean-journal-dark', get_template_directory_uri() . '/css/colors/dark.css', array(), null );
	...
	if ( ! $options['disable_scrollup'] ) {
		wp_enqueue_script( 'clean-journal-scrollup', get_template_directory_uri() . '/js/clean-journal-scrollup.min.js', array( 'jquery' ), '20072014', true  );
	}

	/**
	 * Enqueue custom script for journal.
	 */
	wp_enqueue_script( 'clean-journalcustom-scripts', get_template_directory_uri() . '/js/clean-journal-custom-scripts.min.js', array( 'jquery' ), null );
}
 ...
 * @since Clean Journal 0.1
 */
function clean_journal_enqueue_metabox_scripts() {
    //Scripts
	wp_enqueue_script( 'clean-journalmetabox', get_template_directory_uri() . '/js/clean-journal-metabox.min.js', array( 'jquery', 'jquery-ui-tabs' ), '2013-10-05' );

	//CSS Styles
	wp_enqueue_style( 'clean-journalmetabox-tabs', get_template_directory_uri() . '/css/clean-journal-metabox-tabs.css' );
}
add_action( 'admin_print_scripts-post-new.php', 'clean_journal_enqueue_metabox_scripts', 11 );
add_action( 'admin_print_scripts-post.php', 'clean_journal_enqueue_metabox_scripts', 11 );
add_action( 'admin_print_scripts-page-new.php', 'clean_journal_enqueue_metabox_scripts', 11 );
add_action( 'admin_print_scripts-page.php', 'clean_journal_enqueue_metabox_scripts', 11 );


/**
 * Default Options.
 */
require get_template_directory() . '/inc/clean-journal-default-options.php';

/**
 * Custom Header.
 */
require get_template_directory() . '/inc/clean-journal-custom-header.php';


/**
 * Structure for journal
 */
require get_template_directory() . '/inc/clean-journal-structure.php';


/**
 * Customizer additions.
 */
require get_template_directory() . '/inc/customizer-includes/clean-journal-customizer.php';


/**
 * Custom Menus
 */
require get_template_directory() . '/inc/clean-journal-menus.php';


/**
 * Load Slider file.
 */
require get_template_directory() . '/inc/clean-journal-featured-slider.php';


/**
 * Load Featured Content.
 */
require get_template_directory() . '/inc/clean-journal-featured-content.php';


/**
 * Load Breadcrumb file.
 */
require get_template_directory() . '/inc/clean-journal-breadcrumb.php';


/**
 * Load Widgets and Sidebars
 */
require get_template_directory() . '/inc/clean-journal-widgets.php';


/**
 * Load Social Icons
 */
require get_template_directory() . '/inc/clean-journal-social-icons.php';


/**
 * Load Metaboxes
 */
require get_template_directory() . '/inc/clean-journal-metabox.php';
...
function clean_journal_flush_transients(){
	...
	delete_transient( 'clean_journal_favicon' );

	delete_transient( 'clean_journal_webclip' );
...
if ( ! function_exists( 'clean_journal_favicon' ) ) :
	/**
	 ...
	 * @since Clean Journal 0.1
	 */
	function clean_journal_favicon() {
		if( ( !$clean_journal_favicon = get_transient( 'clean_journal_favicon' ) ) ) {
			$options 	= clean_journal_get_theme_options();
			...
			if ( isset( $options[ 'favicon' ] ) &&  $options[ 'favicon' ] != '' &&  !empty( $options[ 'favicon' ] ) ) {
				...
				$clean_journal_favicon = '<link rel="shortcut icon" href="'.esc_url( $options[ 'favicon' ] ).'" type="image/x-icon" />';
			}

			set_transient( 'clean_journal_favicon', $clean_journal_favicon, 86940 );
		}
		echo $clean_journal_favicon ;
	}
endif; //clean_journal_favicon
//Load Favicon in Header Section
add_action( 'wp_head', 'clean_journal_favicon' );
//Load Favicon in Admin Section
add_action( 'admin_head', 'clean_journal_favicon' );


if ( ! function_exists( 'clean_journal_web_clip' ) ) :
	/**
	 ...
	 * @since Clean Journal 0.1
	 */
	function clean_journal_web_clip() {
		if( ( !$clean_journal_web_clip = get_transient( 'clean_journal_web_clip' ) ) ) {
			$options 	= clean_journal_get_theme_options();

			echo '<!-- refreshing cache -->';

			if ( isset( $options[ 'web_clip' ] ) &&  $options[ 'web_clip' ] != '' &&  !empty( $options[ 'web_clip' ] ) ){
				$clean_journal_web_clip = '<link rel="apple-touch-icon-precomposed" href="'.esc_url( $options[ 'web_clip' ] ).'" />';
			}

			set_transient( 'clean_journal_web_clip', $clean_journal_web_clip, 86940 );
		}
		echo $clean_journal_web_clip ;
	} // clean_journal_web_clips
endif; //clean_journal_web_clip
//Load Clean Journal Icon in Header Section
add_action('wp_head', 'clean_journal_web_clip');
	...
	function clean_journal_content_nav( $nav_id ) {
		...
		?>
	        <nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>">
	        	<h3 class="screen-reader-text"><?php _e( 'Post navigation', 'clean-journal' ); ?></h3>
				<?php
				/**
	...
	function clean_journal_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div class="comment-body">
				<?php _e( 'Pingback:', 'clean-journal' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'clean-journal' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
			...
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				<footer class="comment-meta">
					...
					<div class="comment-metadata">
						...
						<?php edit_comment_link( __( 'Edit', 'clean-journal' ), '<span class="edit-link">', '</span>' ); ?>
					</div><!-- .comment-metadata -->

					<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'clean-journal' ); ?></p>
					<?php endif; ?>
				</footer><!-- .comment-meta -->
...
if ( ! function_exists( 'clean_journal_entry_meta' ) ) :
	...
	function clean_journal_entry_meta() {
		...
		printf( '<span class="posted-on">%1$s<a href="%2$s" rel="bookmark">%3$s</a></span>',
			sprintf( _x( '<span class="screen-reader-text">Posted on</span>', 'Used before publish date.', 'clean-journal' ) ),
...
if ( ! function_exists( 'clean_journal_continue_reading' ) ) :
	...
	function clean_journal_continue_reading() {
		...
		return ' <a class="more-link" href="'. esc_url( get_permalink() ) . '">' .  sprintf( __( '%s', 'clean-journal' ) , $more_tag_text ) . '</a>';
	}
...
if ( ! function_exists( 'clean_journal_body_classes' ) ) :
	...
	function clean_journal_body_classes( $classes ) {
		...
		if ( $page_id == $page_for_posts || $page_id == $page_on_front ) {
	        $layout = get_post_meta( $page_id,'clean-journallayout-option', true );
	    }
    	else if ( is_singular() ) {
	 		if ( is_attachment() ) {
				$parent = $post->post_parent;

				$layout = get_post_meta( $parent,'clean-journallayout-option', true );
			}
			else {
				$layout = get_post_meta( $post->ID,'clean-journallayout-option', true );
			}
		}
		else {
			$layout = 'default';
...
if ( ! function_exists( 'clean_journal_page_post_meta' ) ) :
	...
	function clean_journal_page_post_meta() {
	    ...
	    $clean_journal_page_post_meta .= '<span class="post-author">' . __( 'By', 'clean-journal' ) . ' <span class="author vcard"><a class="url fn n" href="' . $clean_journal_author_url . '" title="View all posts by ' . get_the_author() . '" rel="author">' .get_the_author() . '</a></span>';
